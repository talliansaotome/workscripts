...
if [ "$CHOICE" = "1" ]; then
    ...
    if [[ "$LOCAL" == "false" ]]; then
        time rsync -aHS "$SHAREDOPTS" $REMOTE:$REMOTEPATH/data/ data-REFRESH-"$TICKET"/ || {
            echo "Failed to rsync remote data."
            exit 1
        }
    elif [[ "$LOCAL" == "true" ]]; then
        time rsync -aHS "$SHAREDOPTS" $REMOTEPATH/data/ data-REFRESH-"$TICKET"/ || {
            echo "Failed to rsync data."
            exit 1
        }
    fi
    ln -s data-REFRESH-"$TICKET" next.data || {
    ...
    ## The fancy diffs
    for FILEPAIR in "confluence.cfg.xml-PROD data-REFRESH-$TICKET/confluence.cfg.xml"; do
    ...
    if [[ "$LOCAL" == "true" ]]; then
        ...
        echo "Re-creating and restoring database..."
        ssh $REMOTE 'su - postgres -p -c "dropdb $DATABASE"' || {
            echo "Failed to drop existing database"
            exit 1
        }
        ssh $REMOTE 'su - postgres -p -c "createdb -E UNICODE -O $DATABASEUSERNAME $DATABASE"' || {
            echo "Database creation failed"
            exit 1
        }
        ssh $REMOTE 'PGPASSWORD=$DATABASEPASSWORD; time su - postgres -p -c "zcat /var/lib/pgsql/backups/other/$DATABASE-PRE-$TICKET.dmp.gz | psql -U $DATABASEUSERNAME $DATABASE"' || {
            echo "Database import failed"
            exit 1
        }
    fi
