...
if [ "$CHOICE" = "1" ]; then
	...
	time rsync -aHS "$SHAREDOPTS" data/ data-"$VERSION"/ || {
		echo "Failed to rsync data."
		exit 1
	}
	ln -s data-"$VERSION" next.data || {
		echo "Linking the next data/ dir failed."
	...
	if [[ $APP = confluence ]]; then
		...
		wget -q https://www.atlassian.com/software/confluence/downloads/binary/atlassian-confluence-"$VERSION".tar.gz -O atlassian-confluence-"$VERSION".tar.gz || {
		...
		ln -s atlassian-confluence-"$VERSION" next || {
			echo "Linking the next current/ dir failed"
			read -r -n 1 -p "Press any key to continue ..."
		}
		rm atlassian-confluence-"$VERSION".tar.gz

		for FILEPAIR in 'data/confluence.cfg.xml next.data/confluence.cfg.xml' 'current/bin/setenv.sh next/bin/setenv.sh' 'current/conf/server.xml next/conf/server.xml' 'current/conf/web.xml next/conf/web.xml' 'current/confluence/WEB-INF/classes/confluence-init.properties next/confluence/WEB-INF/classes/confluence-init.properties' 'current/confluence/WEB-INF/classes/seraph-config.xml next/confluence/WEB-INF/classes/seraph-config.xml' 'current/confluence/WEB-INF/classes/crowd.properties next/confluence/WEB-INF/classes/crowd.properties' 'current/confluence/WEB-INF/classes/okta-config-confluence.xml next/confluence/WEB-INF/classes/okta-config-confluence.xml'; do
		...
		echo "Fetching and installing the app"
		wget -q https://www.atlassian.com/software/jira/downloads/binary/atlassian-jira-software-$VERSION.tar.gz -O atlassian-jira-software-$VERSION.tar.gz || {
			echo "Download failed"
			exit 1
		}
	...
	chown -R root:root next/ && chown -R "$FILEOWNER". next/{conf,logs,temp,webapps,work} && chown -R "$FILEOWNER". next.data/ || {
	...
	time rsync -aHS "$SHAREDOPTS" --delete data/ next.data/ || {
		echo "Failed to do final rsync."
		exit 1
	}
	if [[ $APP = confluence ]]; then
		mv -v data/attachments next.data/ || {
			echo "Failed to move attachments."
			read -r -n 1 -p "Press any key to continue ..."
		}
	elif [[ $APP = jira ]]; then
		mv -v data/data/attachments next.data/ || {
			echo "Failed to move attachments."
			read -r -n 1 -p "Press any key to continue ..."
		}
	fi

	mv -v current prev && mv -v next current || {
		echo "Failed to update application symlinks."
		exit 1
	}
	mv -v data prev.data && mv -v next.data data || {
		echo "Failed to update data symlinks."
		exit 1
	}
	...
	if [[ "$DATABASECONNECTION" == *"localhost"* ]]; then
		...
		time su - postgres -c "pg_dump -O $DATABASE | gzip > /var/lib/pgsql/backups/other/$DATABASE-PRE-$TICKET.dmp.gz" || {
		...
		time "pg_dump -H $DATABASEHOST -P $DATABASEPORT -U $DATABASEUSERNAME -O $DATABASE | gzip > /var/lib/pgsql/backups/other/$DATABASE-PRE-$TICKET.dmp.gz" || {
			echo "Database dump failed"
			exit 1
		}
		echo "checking to be sure backup was created"
	...
	if [[ "$DATABASECONNECTION" == *"localhost"* ]]; then
		...
		echo "Re-creating and restoring database..."
		export PGPASSWORD=$DATABASEPASSWORD
		dropdb -h $DATABASEHOST -p $DATABASEPORT -U $DATABASEUSERNAME || {
			echo "Failed to drop existing database"
			exit 1
		}
		su - postgres -p -c "createdb -E UNICODE -O $DATABASEUSERNAME $DATABASE" || {
	...
	## Restore files
	mv -v data/attachments prev.data/ || {
		echo "Failed to move attachments back..."
		exit 1
	}
	mv -v current failed-"$TICKET" && mv -v data failed.data-"$TICKET" || {
		echo "Failed to move symlinks to failed targets..."
		exit 1
	}
	mv -v prev current && mv -v prev.data data || {
		echo "Failed to restore original symlinks..."
		exit 1
	}
